  ...
  return (
    <>
      ...
      <Drawer
        ...
        closable={false}
      >
        <p className="pb-6 text-white font-semibold text-lg">Danh sách yêu thích</p>
          ...
          {wishlist.length === 0 ? (
            ...
            wishlist.map((item) => (
              <div
                ...
                className="flex flex-row justify-between items-center bg-white rounded overflow-hidden pl-4 pr-2 cursor-pointer hover:opacity-80"
                onClick={() => navigate(`/product/${item.equipmentId}`)}
              >
                <img
                  src={item.primaryImageUrl?.trim() || getRandomImage()}
                  className="w-[120px] h-[100px] object-cover p-2"
